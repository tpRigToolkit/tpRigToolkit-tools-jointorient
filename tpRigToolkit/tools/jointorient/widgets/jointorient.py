...
class JointOrientWidget(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        joint_ori_splitter = dividers.Divider('JOINT ORIENT')
        ...
        joint_ori_widget.layout().addLayout(dividers.DividerLayout())
        ...
        manual_joint_ori_splitter = dividers.Divider('MANUAL JOINT ORIENT')
        ...
        manual_joint_splitter_layout.addLayout(dividers.DividerLayout())
        ...
        set_rot_axis_splitter = dividers.Divider('SET ROTATION AXIS')
        ...
        set_rot_axis_splitter_layout.addLayout(dividers.DividerLayout())
    ...
    def orient_joints(self):
        ...
        for jnt in reversed(joints):
            ...
            childs = tp.Dcc.list_children(
                jnt, all_hierarchy=False, children_type=['transform', 'joint'])
    ...
    def set_manual_orient_joints(self):
        ...
        for jnt in joints:
            if not self.manual_joint_ori_set_cbx.isChecked():
                childs = tp.Dcc.list_children(
                    jnt, children_type=['transform', 'joint'], full_path=False, all_hierarchy=False)
                if childs:
                    if len(childs) > 0:
                        for child in childs:
